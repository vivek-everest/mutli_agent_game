...
from games.without_obstacles.environment import GridWorldWithoutObstacles
from agents.without_obstacles.seeker import SeekerAgentWithoutObstacles
from agents.without_obstacles.navigator import NavigatorAgentWithoutObstacles
from games.without_obstacles.state import GameStateWithoutObstacles
from utils.logger import log

def initialize_game():
    st.session_state.env = GridWorldWithoutObstacles()
    st.session_state.seeker = SeekerAgentWithoutObstacles("A0", [0, 0])
    st.session_state.navigator = NavigatorAgentWithoutObstacles("N0")
    st.session_state.state = GameStateWithoutObstacles()
...
def run_game_step():
    ...
    if found:
        log(f"🎉 Treasure found at {new_position}")
        st.session_state.state.set_treasure_found(new_position)
        st.session_state.game_running = False
        st.session_state.game_result = "success"
        return
...
def main():
    ...
    st.title("Treasure Hunt Game (Without Obstacles)")
